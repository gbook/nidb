...
//	require "includes.php";
	...
	$redcapfielSdtime = GetVariable("redcapfieldStime");
...
?>		
	...
	<div class=content>
	   <div class="ui field">
		Select Redcap Form/s (List follows Redcap Sequence)
                <select class="ui search dropdown" name="inst[]" required multiple="multiple">
	 ...
	 <div class=content>
           <div class="ui field">
		Select the type of NiDB data
                <select  class="ui search dropdown"  name="nidbdatatype" required  onchange="this.form.submit()">
	 ...
	 /* -------------------------------------------- */
                ...
                <table class="ui graydisplaytable">
                        <thead>
                                <tr>
                                        ...
                                        <th style="text-align: center" colspan="3">NiDB</th>
                                </tr>
                                <tr>
                                        <th>Event</th>
					<th> Value </th>
					...
					<th> Notes </th>
					<th> Start Time </th>
					<th style="border-right: 1px solid #bdbdbd"> End Time (If Any) </th>
                                        <th>Type</th>
                                        <th>Variable</th>
                                        <th></th>
                                </tr>
					...
					<?}elseif ($nidbdatatype=='d'){?><td><input type="text" disabled value="Drug /Dose"</td><?}?>
                                ...
                                <?
                                        $sqlstring = "select * from redcap_import_mapping where project_id = $projectid and redcap_form IN ('".implode("','", $inst)."') order by formmap_id desc";
                                        $result = MySQLiQuery($sqlstring, __FILE__, __LINE__);
		...
		<form class"=ui form" action="redcapmapping.php" method="post">
			...
			<select name="jointid" required >
			   <option value=''> </option>
			   <?for($In=0;$In < count($In_Name); $In++){ ?>
			     <optgroup label=<?=$In_Name[$In]?>>
				<? $V_names=getrcvariables($projectid,$In_Name[$In],$redcapevent);?>
	                        <?for($Fi=0;$Fi < count($V_names); $Fi++){?>
        	                 <option value=<?=$V_names[$Fi]?>> <?=$V_names[$Fi]?> </option>
				<?}?>
			     </optgroup>
