<?
 ...
 // ------------------------------------------------------------------------------
	...
	function GetSeriesInfo($id, $modality) {
		...
		$path = $GLOBALS['cfg']['archivedir'] . "/$uid/$studynum";
		return array($path, $uid, $studynum, $seriesnum, $seriesdesc, $imagetype, $seriessize, $numfiles, $studyid, $subjectid, $modality, $type, $studydatetime, $enrollmentid, $projectname, $projectid);
	}
	...
	function GetEnrollmentInfo($id) {
		if ($id != "") {
			...
			/* get primary alternate ID */
			$sqlstring = "select * from subject_altuid where subject_id = '$subjectid' and enrollment_id = $id and isprimary = 1";
			$result = MySQLiQuery($sqlstring,__FILE__,__LINE__);
			$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
			$altuid = trim($row['altuid']);
			//if ($altuid == "")
			//	$altuid = $uid;
			//echo "Primary alternate UID for [$uid] is [$altuid]<br>";
			
			return array($uid, $subjectid, $altuid, $projectname, $projectid);
		}
		else {
			return array("", -1, "", "", -1);
		}
...
function myErrorHandler($errno, $errstr, $errfile, $errline)
{
	...
	/* ------- IsViewablePath --------------------- */
	/* -------------------------------------------- */
	/* true if $realfile (already passed through realpath()) lives within the
	   archive, mount, or an analysis directory. Used by viewfile.php and getfile.php */
	function IsViewablePath($realfile) {
		if (($realfile === false) || ($realfile == '')) { return false; }
	...
	function DisplaySettings($returnpage) {
		...
		<div class="ui container">
			<div class="ui fluid styled accordion">
				...
				<div class="<?=$active?> content">
					...
					<table class="ui very compact top attached celled table">
						<thead>
							<tr>
								<th class="ui inverted header" style="background-color: #666">Variable</th>
								<th class="ui inverted header" style="background-color: #666">Value</th>
								<th class="ui inverted header" style="background-color: #666">Valid?</th>
								<th class="ui inverted header" style="background-color: #666">Description</th>
							</tr>
						</thead>
						<tr>
			...
			<div class="ui fluid styled accordion">
				<div class="title">
					<i class="dropdown icon"></i> PHP Variables
				</div>
				<div class="content">
