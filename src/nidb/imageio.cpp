...
/* ---------------------------------------------------------- */
 bool imageIO::AnonymizeDicomFile(QString infile, QString outfile, QString &msg) {
    ...
    if (status.good()) {
        ...
        if (dataset->tagExists(DCM_ReferringPhysicianName)) { status = dataset->putAndInsertString(DCM_ReferringPhysicianName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_ReferringPhysicianName]\n"; }
        if (dataset->tagExists(DCM_PerformingPhysicianName)) { status = dataset->putAndInsertString(DCM_PerformingPhysicianName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PerformingPhysicianName]\n"; }
        if (dataset->tagExists(DCM_OperatorsName)) { status = dataset->putAndInsertString(DCM_OperatorsName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_OperatorsName]\n"; }
        if (dataset->tagExists(DCM_PatientName)) { status = dataset->putAndInsertString(DCM_PatientName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientName]\n"; }
        if (dataset->tagExists(DCM_PatientBirthDate)) { status = dataset->putAndInsertString(DCM_PatientBirthDate, anonDate); if (status.bad()) msg += "Error changing tag [DCM_PatientBirthDate]\n"; }
        ...
        if (status.good()) {
            ...
            switch (anonlevel) {
                ...
                case 2: {
                    ...
                    if (dataset->tagExists(DCM_InstanceCreationDate)) { status = dataset->putAndInsertString(DCM_InstanceCreationDate, anonDate); if (status.bad()) msg += "Error changing tag [DCM_InstanceCreationDate]\n"; }
                    if (dataset->tagExists(DCM_InstanceCreationTime)) { status = dataset->putAndInsertString(DCM_InstanceCreationTime, anonTime); if (status.bad()) msg += "Error changing tag [DCM_InstanceCreationTime]\n"; }
                    if (dataset->tagExists(DCM_StudyDate)) { status = dataset->putAndInsertString(DCM_StudyDate, anonDate); if (status.bad()) msg += "Error changing tag [DCM_StudyDate]\n"; }
                    if (dataset->tagExists(DCM_SeriesDate)) { status = dataset->putAndInsertString(DCM_SeriesDate, anonDate); if (status.bad()) msg += "Error changing tag [DCM_SeriesDate]\n"; }
                    if (dataset->tagExists(DCM_AcquisitionDate)) { status = dataset->putAndInsertString(DCM_AcquisitionDate, anonDate); if (status.bad()) msg += "Error changing tag [DCM_AcquisitionDate]\n"; }
                    if (dataset->tagExists(DCM_ContentDate)) { status = dataset->putAndInsertString(DCM_ContentDate, anonDate); if (status.bad()) msg += "Error changing tag [DCM_ContentDate]\n"; }
                    if (dataset->tagExists(DCM_StudyTime)) { status = dataset->putAndInsertString(DCM_StudyTime, anonTime); if (status.bad()) msg += "Error changing tag [DCM_StudyTime]\n"; }
                    if (dataset->tagExists(DCM_SeriesTime)) { status = dataset->putAndInsertString(DCM_SeriesTime, anonTime); if (status.bad()) msg += "Error changing tag [DCM_SeriesTime]\n"; }
                    if (dataset->tagExists(DCM_AcquisitionTime)) { status = dataset->putAndInsertString(DCM_AcquisitionTime, anonTime); if (status.bad()) msg += "Error changing tag [DCM_AcquisitionTime]\n"; }
                    if (dataset->tagExists(DCM_ContentTime)) { status = dataset->putAndInsertString(DCM_ContentTime, anonTime); if (status.bad()) msg += "Error changing tag [DCM_ContentTime]\n"; }
                    if (dataset->tagExists(DCM_InstitutionName)) { status = dataset->putAndInsertString(DCM_InstitutionName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_InstitutionName]\n"; }
                    if (dataset->tagExists(DCM_InstitutionAddress)) { status = dataset->putAndInsertString(DCM_InstitutionAddress, anonStr); if (status.bad()) msg += "Error changing tag [DCM_InstitutionAddress]\n"; }
                    if (dataset->tagExists(DCM_ReferringPhysicianName)) { status = dataset->putAndInsertString(DCM_ReferringPhysicianName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_ReferringPhysicianName]\n"; }
                    if (dataset->tagExists(DCM_ReferringPhysicianAddress)) { status = dataset->putAndInsertString(DCM_ReferringPhysicianAddress, anonStr); if (status.bad()) msg += "Error changing tag [DCM_ReferringPhysicianAddress]\n"; }
                    if (dataset->tagExists(DCM_ReferringPhysicianTelephoneNumbers)) { status = dataset->putAndInsertString(DCM_ReferringPhysicianTelephoneNumbers, anonStr); if (status.bad()) msg += "Error changing tag [DCM_ReferringPhysicianTelephoneNumbers]\n"; }
                    if (dataset->tagExists(DCM_ReferringPhysicianIdentificationSequence)) { status = dataset->putAndInsertString(DCM_ReferringPhysicianIdentificationSequence, anonStr); if (status.bad()) msg += "Error changing tag [DCM_ReferringPhysicianIdentificationSequence]\n"; }
                    if (dataset->tagExists(DCM_StationName)) { status = dataset->putAndInsertString(DCM_StationName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_StationName]\n"; }
                    if (dataset->tagExists(DCM_StudyDescription)) { status = dataset->putAndInsertString(DCM_StudyDescription, anonStr); if (status.bad()) msg += "Error changing tag [DCM_StudyDescription]\n"; }
                    if (dataset->tagExists(DCM_SeriesDescription)) { status = dataset->putAndInsertString(DCM_SeriesDescription, anonStr); if (status.bad()) msg += "Error changing tag [DCM_SeriesDescription]\n"; }
                    if (dataset->tagExists(DCM_PhysiciansOfRecord)) { status = dataset->putAndInsertString(DCM_PhysiciansOfRecord, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PhysiciansOfRecord]\n"; }
                    if (dataset->tagExists(DCM_PerformingPhysicianName)) { status = dataset->putAndInsertString(DCM_PerformingPhysicianName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PerformingPhysicianName]\n"; }
                    if (dataset->tagExists(DCM_NameOfPhysiciansReadingStudy)) { status = dataset->putAndInsertString(DCM_NameOfPhysiciansReadingStudy, anonStr); if (status.bad()) msg += "Error changing tag [DCM_NameOfPhysiciansReadingStudy]\n"; }
                    if (dataset->tagExists(DCM_OperatorsName)) { status = dataset->putAndInsertString(DCM_OperatorsName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_OperatorsName]\n"; }

                    if (dataset->tagExists(DCM_PatientName)) { status = dataset->putAndInsertString(DCM_PatientName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientName]\n"; }
                    if (dataset->tagExists(DCM_PatientID)) { status = dataset->putAndInsertString(DCM_PatientID, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientID]\n"; }
                    if (dataset->tagExists(DCM_IssuerOfPatientID)) { status = dataset->putAndInsertString(DCM_IssuerOfPatientID, anonStr); if (status.bad()) msg += "Error changing tag [DCM_IssuerOfPatientID]\n"; }
                    if (dataset->tagExists(DCM_PatientBirthDate)) { status = dataset->putAndInsertString(DCM_PatientBirthDate, anonDate); if (status.bad()) msg += "Error changing tag [DCM_PatientBirthDate]\n"; }
                    if (dataset->tagExists(DCM_PatientBirthTime)) { status = dataset->putAndInsertString(DCM_PatientBirthTime, anonTime); if (status.bad()) msg += "Error changing tag [DCM_PatientBirthTime]\n"; }
                    if (dataset->tagExists(DCM_PatientInsurancePlanCodeSequence)) { status = dataset->putAndInsertString(DCM_PatientInsurancePlanCodeSequence, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientInsurancePlanCodeSequence]\n"; }
                    if (dataset->tagExists(DCM_OtherPatientIDsSequence)) { status = dataset->putAndInsertString(DCM_OtherPatientIDsSequence, anonStr); if (status.bad()) msg += "Error changing tag [DCM_OtherPatientIDsSequence]\n"; }
                    if (dataset->tagExists(DCM_OtherPatientNames)) { status = dataset->putAndInsertString(DCM_OtherPatientNames, anonStr); if (status.bad()) msg += "Error changing tag [DCM_OtherPatientNames]\n"; }
                    if (dataset->tagExists(DCM_PatientBirthName)) { status = dataset->putAndInsertString(DCM_PatientBirthName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientBirthName]\n"; }
                    if (dataset->tagExists(DCM_PatientAge)) { status = dataset->putAndInsertString(DCM_PatientAge, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientAge]\n"; }
                    if (dataset->tagExists(DCM_PatientSize)) { status = dataset->putAndInsertString(DCM_PatientSize, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientSize]\n"; }
                    if (dataset->tagExists(DCM_PatientWeight)) { status = dataset->putAndInsertString(DCM_PatientWeight, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientWeight]\n"; }
                    if (dataset->tagExists(DCM_PatientAddress)) { status = dataset->putAndInsertString(DCM_PatientAddress, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientAddress]\n"; }
                    if (dataset->tagExists(DCM_PatientMotherBirthName)) { status = dataset->putAndInsertString(DCM_PatientMotherBirthName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientMotherBirthName]\n"; }
                    if (dataset->tagExists(DCM_PatientTelephoneNumbers)) { status = dataset->putAndInsertString(DCM_PatientTelephoneNumbers, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientTelephoneNumbers]\n"; }
                    if (dataset->tagExists(DCM_AdditionalPatientHistory)) { status = dataset->putAndInsertString(DCM_AdditionalPatientHistory, anonStr); if (status.bad()) msg += "Error changing tag [DCM_AdditionalPatientHistory]\n"; }
                    if (dataset->tagExists(DCM_PatientReligiousPreference)) { status = dataset->putAndInsertString(DCM_PatientReligiousPreference, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientReligiousPreference]\n"; }
                    if (dataset->tagExists(DCM_PatientComments)) { status = dataset->putAndInsertString(DCM_PatientComments, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PatientComments]\n"; }

                    if (dataset->tagExists(DCM_ProtocolName)) { status = dataset->putAndInsertString(DCM_ProtocolName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_ProtocolName]\n"; }

                    if (dataset->tagExists(DCM_RequestingPhysician)) { status = dataset->putAndInsertString(DCM_RequestingPhysician, anonStr); if (status.bad()) msg += "Error changing tag [DCM_RequestingPhysician]\n"; }
                    if (dataset->tagExists(DCM_RequestedProcedureDescription)) { status = dataset->putAndInsertString(DCM_RequestedProcedureDescription, anonStr); if (status.bad()) msg += "Error changing tag [DCM_RequestedProcedureDescription]\n"; }

                    if (dataset->tagExists(DCM_ScheduledPerformingPhysicianName)) { status = dataset->putAndInsertString(DCM_ScheduledPerformingPhysicianName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_ScheduledPerformingPhysicianName]\n"; }
                    if (dataset->tagExists(DCM_PerformedProcedureStepStartDate)) { status = dataset->putAndInsertString(DCM_PerformedProcedureStepStartDate, anonDate); if (status.bad()) msg += "Error changing tag [DCM_PerformedProcedureStepStartDate]\n"; }
                    if (dataset->tagExists(DCM_PerformedProcedureStepStartTime)) { status = dataset->putAndInsertString(DCM_PerformedProcedureStepStartTime, anonTime); if (status.bad()) msg += "Error changing tag [DCM_PerformedProcedureStepStartTime]\n"; }
                    if (dataset->tagExists(DCM_PerformedProcedureStepID)) { status = dataset->putAndInsertString(DCM_PerformedProcedureStepID, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PerformedProcedureStepID]\n"; }
                    if (dataset->tagExists(DCM_PerformedProcedureStepDescription)) { status = dataset->putAndInsertString(DCM_PerformedProcedureStepDescription, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PerformedProcedureStepDescription]\n"; }
                    if (dataset->tagExists(DCM_HumanPerformerOrganization)) { status = dataset->putAndInsertString(DCM_HumanPerformerOrganization, anonStr); if (status.bad()) msg += "Error changing tag [DCM_HumanPerformerOrganization]\n"; }
                    if (dataset->tagExists(DCM_HumanPerformerName)) { status = dataset->putAndInsertString(DCM_HumanPerformerName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_HumanPerformerName]\n"; }
                    if (dataset->tagExists(DCM_PersonName)) { status = dataset->putAndInsertString(DCM_PersonName, anonStr); if (status.bad()) msg += "Error changing tag [DCM_PersonName]\n"; }
