...
int modulePipeline::Run() {
    ...
    while (q.next()) {
        ...
        /* ------------------------------ level 0 ----------------------------- */
        /* Level 0 is not maintained - ignore this section                      */
        /* -------------------------------------------------------------------- */
        if (p.level == 0) {
            SetPipelineStopped(pipelineid, "Pipeline module stopped. Level 0 pipelines have been deprecated.");
            SetPipelineProcessStatus("complete",0,0);
            continue;
            ...
            if (n->SubmitClusterJob(jobFilePath, p.clusterType, p.clusterSubmitHost, p.clusterSubmitHostUser, n->cfg["qsubpath"], p.clusterUser, p.clusterQueue, qm, jobid, qresult)) {
                ...
                n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupSubmitToCluster, LogStatus::success, 0, "","");
            }
            else {
        ...
        // ================================= LEVEL 1 =================================
            ...
            if (p.directory == "")
                ...
                pipelinedirectory = n->cfg["mountdir"] + p.directory;
            ...
            foreach (int sid, studyids) {
                ...
                study s(sid, n);
                if (!s.isValid()) {
                    n->Log("Study was not valid [" + s.msg + "]. Maybe this study was deleted after this pipeline started?");
                ...
                do {
                    ...
                    if (filled == 2) {
                        n->Log("Pipeline max concurrent jobs is not set", __FUNCTION__);
                        RecordPipelineEvent(pipelineid, runnum, -1, "errorNoMaxConcurrentJobs", "max concurrent jobs not set");
                        SetPipelineStopped(pipelineid, "Incorrect pipeline config - max concurrent jobs not set");
                        SetPipelineProcessStatus("complete",0,0);
                        return 1;
                    }
                } while (filled == 1);
                ...
                if ((a.runSupplement) || (a.rerunResults) || (analysisRowID == -1)) {
                    ...
                    if (okToRun) {
                        n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupCheckIfOkToRun, LogStatus::success, 0, "","");
                        ...
                        if ((!a.rerunResults) && (!a.runSupplement)) {
                            if (pipelinedep != -1) {
                                setuplog << n->Log(QString("[%1] This pipeline depends on [%2]").arg(p.name).arg(pipelinedep), __FUNCTION__);
                                ...
                                if (q2.size() > 0) {
                                    ...
                                    break; /* leave the study loop */
                                }
                                n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDependencyCheck, LogStatus::success, 0, "Parent pipeline exists", "");
                            }
                            else {
                                setuplog << n->Debug(QString("This pipeline does not depend on any pipelines [%1]").arg(pipelinedep));
                            }

                            QString analysisPipelinePath = analysispath + "/pipeline";
                            if (!MakePath(analysisPipelinePath, m)) {
                                QString errMsg = QString("Error creating directory [%1]").arg(analysisPipelinePath);
                                n->Log(errMsg, __FUNCTION__);
                                n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupCreateDirectory, LogStatus::error, 0, errMsg, "");
                                UpdateAnalysisStatus(analysisRowID, "error", errMsg, 0, -1, "", false, true, -1, -1);
                                continue;
                            }
                            else {
                            ...
                            if (pipelinedep != -1) {
                                ...
                                n->InsertAnalysisEvent(analysisRowID, pipelineid, p.version, sid, "analysismessage", "success", QString("Parent pipeline (dependency) will be copied to directory [%1] using method [%2]").arg(p.depDir).arg(p.depLinkType));
                        ...
                        /* number submitted, regardless of success/failure */
                        numsubmitted++;
                        totalSubmitted++;
                        jobsWereSubmitted = true;
                        ...
                        UpdateAnalysisStatus(analysisRowID, "", "", -1, -1, datalog, false, false, -1, -1);
         ...
         * version, so it's been deprecated and removed in the C++ version of NiDB */
            ...
            n->Log("Level 2 (group) pipelines are not implemented in NiDB", __FUNCTION__);
        }
        else {
...
bool modulePipeline::GetData(int studyid, QString analysispath, QString uid, qint64 analysisRowID, int pipelineid, int pipelinedep, QString deplevel, QList<dataDefinitionStep> datadef, int &numdownloaded, QString &datalog) {
    ...
    for (int i = 0; i < datadef.size(); i++) {
        ...
        if (!enabled) {
            dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepCheck, LogStatus::neutral, stepNum, QString("Step %1 is not enabled. Skipped").arg(stepNum), "");
            continue;
        }
        ...
        if (optional) {
            ...
            n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepCheck, LogStatus::neutral, stepNum, QString("Step %1 optional. Skipped").arg(stepNum), "");
            continue;
        }
        ...
        if (!n->isValidNiDBModality(modality)) {
            dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepCheck, LogStatus::error, stepNum, "Modality [" + modality + "] is not valid", "");
            stepIsInvalid = true;
            break;
        }
        ...
        /* separate any protocols that are actually lists of protocols */
        QStringList prots;
        if (protocol.contains("\""))
            prots = ShellWords(protocol);
        else
            prots << protocol;
        QString protocols = n->BuildInClause("prot", prots, binds);
        if (prots.size() < 1) {
            dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepCheck, LogStatus::error, stepNum, "No protocols specified", "");
            stepIsInvalid = true;
            break;
        }

        /* separate image types */
        QStringList types = imagetype.split(QRegularExpression(",\\s*"), Qt::SkipEmptyParts);
        QString imagetypes = n->BuildInClause("imgtype", types, binds);
        ...
        if (level == "subject") {
            ...
            if ((assoctype == "nearesttime") || (assoctype == "nearestintime")) {
                ...
                sqlstring += QString(" ORDER BY ABS( DATEDIFF( `%1_series`.series_datetime, :studydatetime ) ) LIMIT 1").arg(modality);

                q.prepare(sqlstring);
                q.bindValue(":subjectid", subjectid);
                q.bindValue(":studydatetime", studydate);
                n->BindValues(q, binds);
            }
        ...
        /* otherwise, check the study for the protocol(s) */
            ...
            QString sqlstring;
            dlog << QString("   Checking the study [%1] for the protocol (%2)").arg(studyid).arg(prots.join(", "));
    ...
    if ((stepIsInvalid) && (deplevel == "subject")) {
        ...
        dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataCheckSummary, LogStatus::error, 0, "One of the required steps was invalid because no data was found based on the search criteria. (This was a subject-level dependency) No data was downloaded", "");
        datalog = dlog.join("\n");
        return false;
    }
    ...
    if (stepIsInvalid) {
        ...
        dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataCheckSummary, LogStatus::error, 0, "One of the required steps was invalid because no data was found based on the search criteria. No data was downloaded", "");
        datalog = dlog.join("\n");
        return false;
    }
    ...
    for (int i = 0; i < datadef.size(); i++) {
        ...
        if (prots.size() < 1) {
            dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepDownload, LogStatus::error, stepNum, "No protocols specified", "");
            continue;
        }
        ...
        /* make sure the requested modality table exists */
        if (!n->isValidNiDBModality(modality)) {
            dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepDownload, LogStatus::error, stepNum, "Modality [" + modality + "] is not valid", "");
            continue;
        }
        ...
        /* get a list of series satisfying the search criteria, if it exists */
        if (level == "study") {
            dlog << "\tGetting list of series that match this (STUDY-level) data step -  protocols [" + prots.join(", ") + "]  criteria [" + criteria + "]  imagetype [" + imagetype + "]";
            ...
            dlog << "\tGetting list of series matching this (SUBJECT-level) data step -  protocols [" + prots.join(", ") + "]  criteria [" + criteria + "]  imagetype [" + imagetype + "]";
            ...
            if ((assoctype == "nearesttime") || (assoctype == "nearestintime")) {
                ...
                sqlstringA += QString(" ORDER BY ABS( DATEDIFF( `%1_series`.series_datetime, :studydatetime ) ) LIMIT 1").arg(modality);
                q2.prepare(sqlstringA);
                q2.bindValue(":subjectid", s.GetSubjectRowID());
                q2.bindValue(":studydatetime", s.datetime.toString("yyyy-MM-dd hh:mm:ss"));
                n->BindValues(q2, binds);
                ...
                dlog << n->Log(QString("Preparing (subject-level) data search:  protocols [%1]  criteria [%2]  imagetype [%3]").arg(prots.join(", ")).arg(criteria).arg(types.join(", ")), __FUNCTION__);
        ...
        if (exportBIDS) {
            ...
            if (q.size() > 0) {
                ...
                dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepDownload, LogStatus::neutral, stepNum, QString("Added %1 series to BIDS to-do list").arg(q.size()), "");
            }
            else {
            ...
            if (q.size() > 0) {
                ...
                if (!MakePath(analysispath + "/pipeline", m)) {
                    ...
                    dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupCreateDirectory, LogStatus::error, 0, "Error creating directory [" + analysispath + "/pipeline]","");
                    continue;
                }
                ...
                while (q.next()) {
                    ...
                    if (!behonly) {
                        ...
                        if ((dataformat == "dicom") || ((datatype != "dicom") && (datatype != "parrec"))) {
                            ...
                            if (p.dataCopyMethod == "scp")
                                systemstring = QString("scp %1/* %2\\@%3:%4").arg(indir).arg(clusteruser).arg(submithost).arg(newanalysispath);
                            else
                            ...
                            if (!img->ConvertDicom(dataformat, indir, tmpdir, binpath, gzip, false, uid, localStudyNumStr, seriesNumStr, "", "", mapping, datatype, QJsonObject(), numfilesconv, numfilesrenamed, m)) {
                                dlog << n->Log("Error converting DICOM files [" + m + "]", __FUNCTION__);
                                UpdateAnalysisStatus(analysisRowID, "error", "Error converting DICOM files [" + m + "]", 0, -1, "", false, true, -1, -1);
                                return false;
                            }

                            QString systemstring;
                            if (p.dataCopyMethod == "scp")
                                systemstring = QString("scp %1/* %2\\@%3:%4").arg(tmpdir).arg(clusteruser).arg(submithost).arg(newanalysispath);
                            else
                    ...
                    if (behformat != "behnone") {
                        ...
                        if (!MakePath(behoutdir, m)) {
                            ...
                            UpdateAnalysisStatus(analysisRowID, "error", "Unable to create directory [" + newanalysispath + "]", 0, -1, "", false, true, -1, -1);
                            dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepDownload, LogStatus::error, stepNum, "Error creating behavioral data directory [" + behoutdir + "]", "");
                        }
                        else {
                ...
                if (optional)
                    dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepDownload, LogStatus::neutral, stepNum, "Data not found (step is optional)", "");
                else
                    dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataStepDownload, LogStatus::error, stepNum, "No matching series", "");
            }
        }
    ...
    if (exportBIDS) {
        if (BIDSseriesids.size() > 0) {
            ...
            dlog << n->LogAnalysisEvent(analysisRowID, AnalysisEvent::SetupDataDownloadSummary, LogStatus::error, 0, "No series found for export in BIDS format", "");
            datalog = dlog.join("\n");
            return false;
        }
    }
...
QStringList modulePipeline::GetGroupList(int pid) {
    ...
    if (q.size() > 0) {
        ...
        /* pipeline_groupid is a comma-separated list. Keep only valid integer IDs */
        QStringList groupids;
        foreach (QString id, q.value("pipeline_groupid").toString().split(",", Qt::SkipEmptyParts)) {
            id = id.trimmed();
            if (IsInt(id))
                groupids << id;
        }
        ...
        if (groupids.size() > 0) {
            QVariantMap binds;
            QSqlQuery q2;
            q2.prepare("select group_name from groups where group_id in (" + n->BuildInClause("groupid", groupids, binds) + ")");
            n->BindValues(q2, binds);
            n->SQLQuery(q2, __FUNCTION__, __FILE__, __LINE__);
            while (q2.next())
                grouplist.append(q2.value("group_name").toString());
        }
    }
...
void modulePipeline::RecordPipelineEvent(int pipelineid, qint64 &runnum, qint64 analysisid, QString event, QString message) {
    ...
    */
    ...
    /* do an insert */
    if (analysisid > 0) {
        q.prepare("insert into pipeline_history (run_num, pipeline_id, pipeline_version, analysis_id, pipeline_event, event_message) values (:runnum, :pipelineid, :version, :analysisid, :event, :msg)");
        q.bindValue(":analysisid", analysisid);
    }
    else {
