...
# -----------------------------------------------------------------------------
# This program reads from a special eeg incoming directory, populates the database, and
...
our $scriptname = "parseincomingeeg";
our $lockfileprefix = "parseincomingeeg";	# lock files will be numbered lock.1, lock.2 ...
...
sub DoParse {
	...
	my $ret = 0;
	my $i = 0;
	my $costcenter = "126237";
	my $datadir = "/mount/mammoth/data2/erp_lab/Memantine";
	my $alternateuid;
	...
	foreach my $dir (@dirs) {
		if ( ($dir ne ".") && ($dir ne "..") ) {
			if (-d "$datadir/$dir") {
				...
				if ($alternateuid =~ /^M0[0-9]{7}_DOSE$/) {
					$alternateuid =~ s/\_DOSE//g;
...
sub InsertSeries {
	...
	foreach my $file (@files) {
		if ( ($file ne ".") && ($file ne "..") ) {
			...
			if (($ext eq "cnt") || ($ext eq "3dd") || ($ext eq "dat")) {
				my ($junk, $studydate, $taskdesc, $operator) = split(/_/, $file);
				print "$filepath [[$ext]]\n";
				switch ($ext) {
					case 'cnt' { $SeriesNumber = $i;
								if (uc(trim($taskdesc)) eq "G") { $SeriesDesc = "Gating"; }
								if (uc(trim($taskdesc)) eq "OB") { $SeriesDesc = "Oddball"; }
								if (uc(trim($taskdesc)) eq "EYESOPEN") { $SeriesDesc = "Rest - eyes open"; }
								if (uc(trim($taskdesc)) eq "EYESCLOSED") { $SeriesDesc = "Rest - eyes closed"; }
								#$SeriesDesc = $taskdesc;
								#print "CNT file: $filepath\n";
								$i++;
							}
					case 'dat' { $SeriesNumber = 0; $SeriesDesc = "Polhemus"; }
					case '3dd' { $SeriesNumber = 0; $SeriesDesc = "Polhemus"; }
				}
				
				# insert the series, using the info from the previous operations
				my $sqlstring3 = "select * from eeg_series where study_id = $studyRowID and series_datetime = '$SeriesDateTime' and series_num = $SeriesNumber and series_protocol = '$SeriesDesc'";
				my $result3 = $db->query($sqlstring3) || SQLError($db->errmsg(),$sqlstring3);
				if ($result3->numrows > 0) {
					my %row3 = $result3->fetchhash;
					$seriesRowID = $row3{'eegseries_id'};
				}
				else {
					$sqlstring = "insert into eeg_series (study_id, series_datetime, series_num, series_protocol) values ($studyRowID, '$SeriesDateTime', $SeriesNumber, '$SeriesDesc')";
					print "[$sqlstring]\n";
					my $result2 = $db->query($sqlstring) || SQLError($db->errmsg(),$sqlstring);
				...
				my $outdir;
				$outdir = "$cfg{'archivedir'}/$subjectRealUID/$studyNum/$SeriesNumber/eeg";
				...
				$sqlstring = "update eeg_series set series_size = $dirsize, series_numfiles = $numfiles where eegseries_id = $seriesRowID";
				$result = $db->query($sqlstring) || SQLError($db->errmsg(),$sqlstring);
