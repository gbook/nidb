...
void analysis::LoadAnalysisInfo() {
	...
	QSqlQuery q;
	q.prepare("select d.uid, b.study_num, b.study_id, e.pipeline_name, e.pipeline_id, e.pipeline_version, e.pipeline_dirstructure from analysis a left join studies b on a.study_id = b.study_id left join enrollment c on b.enrollment_id = c.enrollment_id left join subjects d on c.subject_id = d.subject_id left join pipelines e on a.pipeline_id = e.pipeline_id where a.analysis_id = :analysisid");
	...
	if ((analysispath == "") || (analysispath == ".") || (analysispath == "..") || (analysispath == "/") || analysispath.contains("//")) {
