#!/bin/bash
# ------------------------------------------------------------------------------
# NiDB Debian 13 (trixie) package builder
...
#
# Same as the Debian 12 builder except for trixie-specific package names:
#   - 7zip replaces the now-transitional p7zip-full
# trixie ships PHP 8.4 and MariaDB 11.8; the postinst detects the PHP version at
# runtime, so no changes are needed there.
# ------------------------------------------------------------------------------
set -e
cd "$(dirname "$0")"
...
Depends: apache2, mariadb-server, mariadb-client, mariadb-backup, libmariadb3, php-fpm, php-cli, php-mysql, php-gd, php-mbstring, php-opcache, php-curl, imagemagick, libimage-exiftool-perl, openssl, zip, unzip, 7zip, default-jre, libgl1, libegl1, libx11-6, libxkbcommon0, libfontconfig1, libfreetype6, libglib2.0-0, shellcheck
