			...
			while ($rowA = mysqli_fetch_array($resultA, MYSQLI_ASSOC)) {
				...
				if (file_exists("$archivedir/$uid/$studynum")) {
					...
					while ($rowB = mysqli_fetch_array($resultB, MYSQLI_ASSOC)) {
						...
						$datatype = $rowB['data_type'];
						
						/* check if the subject directory exists */
						if (file_exists("$archivedir/$uid/$studynum/$seriesnum")) {
							if (file_exists("$archivedir/$uid/$studynum/$seriesnum/$datatype")) {
								chdir("$archivedir/$uid/$studynum/$seriesnum/$datatype");
								$count = count(glob("*.*"));
								if ($count != $numfiles) {
									echo "$uid\t$studynum\t$seriesnum\t$modality\tFileCountMismatch DB: [$numfiles] FS: [$count]\n";
								}
							}
							else {
								echo "$uid\t$studynum\t$seriesnum\t$modality\tDataDirectoryMissing [$datatype]\n";
							}
						}
						else {
							echo "$uid\t$studynum\t$seriesnum\t$modality\tMissingFromFileSystem\n";
						}
					}
				}
				else {
					echo "$uid\t$studynum\t-\t$modality\tMissingFromFileSystem\n";
				}
			}
		}
		else {
			echo "$uid\t-\t-\t-\tMissingFromFileSystem\n";
		}
	}
