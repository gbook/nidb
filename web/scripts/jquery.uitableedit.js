...
jQuery.uiTableEdit = function(jq, options){
	...
	function td_edit(){
		...
		var	w =	td.width();
		var	h =	td.height();
		//td.css({width: w + "px", height: h + "px", padding: "0", margin: "0",	'background-color' : 'aliceblue'});
		//td.css({'color' : 'blue'});
		td.css({'font-weight' : 'bold'});
		td.html( '<form	name="td-editor" action="javascript:void(0);">'	+
		'<input type="text" size="5" id="td_edit"	name="td_edit" value="'	+ td.text()	+ '"' +	' style="color:red;font-size:10pt;padding:0px;margin:0px">'	+ '</input></form>'	)
		.find('form').submit(	restore	).mousedown( restore ).blur( restore ).keypress( checkEscape );
		document.getElementById('td_edit').select();
