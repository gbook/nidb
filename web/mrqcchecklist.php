...
<?
	...
	function DisplayProjectChecklist($projectid) {
		...
		<input type="hidden" name="returnpage" value="mrqcchecklist.php?projectid=<?=$projectid?>">
				...
				foreach ($checklist as $i => $item) {
					...
					if ($msg == "") {
						...
						if (mysqli_num_rows($result) > 0) {
							...
							?><td style="border: 1px solid #ffd699; background-image: repeating-linear-gradient(-45deg, transparent, transparent 5px, #ddd 5px, #ddd 10px);" title="<b><?=$reason?></b> - <?=$date?>"><a href="mrqcchecklist.php?action=setmissingdatareasonform&missingdataid=<?=$missingdataid?>&projectid=<?=$projectid?>&enrollmentid=<?=$enrollmentid?>&projectchecklistid=<?=$itemid?>&reason=<?=$reason?>">&#10006;</a></td><?
						}
						else {
							?><td style="border: 1px solid #ffd699; background-image: repeating-linear-gradient(45deg, transparent, transparent 5px, #ffe0b3 5px, #ffe0b3 10px);" title="Click to set reason for missing data"><a href="mrqcchecklist.php?action=setmissingdatareasonform&projectid=<?=$projectid?>&enrollmentid=<?=$enrollmentid?>&projectchecklistid=<?=$itemid?>">?</a></td><?
						}
					}
					else {
